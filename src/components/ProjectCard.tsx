...
export default function ProjectCard({ 
  ...
  const tall = isTall !== undefined ? isTall : project.isTall;
  const wide = isWide !== undefined ? isWide : false;
  const spanClass = `${tall ? 'row-span-2' : 'row-span-1'} ${wide ? 'col-span-2' : 'col-span-1'}`;

  useGSAP(() => {
    // Create a master timeline for the hover state
    ...
    >
      ...
      >
        <div className="text-center p-6 flex flex-col items-center">
          ...
          <div 
            ref={lineRef}
            className="w-0 h-[2px] bg-brand mb-4"
          ></div>
          ...
          <div 
            ref={accent1Ref}
            className="absolute bottom-6 right-6 w-10 h-10 border-b-2 border-r-2 border-brand opacity-0"
          ></div>
          <div 
            ref={accent2Ref}
            className="absolute top-6 left-6 w-10 h-10 border-t-2 border-l-2 border-brand opacity-0"
          ></div>
        </div>
      </div>
    </div>
